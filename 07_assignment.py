...
def exercise02(new_observations):
    ...
    iris_predictions =[exercise01()[1][n] for n in list(y_pred)]
...
def exercise03(neighbors,split):
    ...
    X = iris.data
    y = iris.target 
    X_train, X_test, y_train, y_test = tts(X, y, stratify=y, test_size=split, random_state=random_state)
...
def exercise04():
    ...
    X = iris.data
    y = iris.target 
    X_train, X_test, y_train, y_test = tts(X, y, stratify=y, test_size=.25, random_state=21)
    knn_score = []
    ...
    fig = go.Figure(data=go.Scatter(x=neighbors, y=knn_score))
    fig.write_html('knn_scores.html', auto_open=True)
    plotly_overfit_underfit_curve_url = 'knn_scores.html'
    # ------ Place code above here /\ /\ /\ ------
...
def exercise05():
    ...
    df_tv = pd.DataFrame(boston.target, columns = ['MEDV'])
    average_age = df_data['AGE'].mean()
    average_medv = df_tv.mean()
    medv_as_numpy_values = np.array(boston.target).reshape(-1,1)
...
def exercise06():
    ...
    df_data = pd.DataFrame(boston.data, columns = boston.feature_names)
    df_tv = pd.DataFrame(boston.target, columns = ['MEDV'])
    plt.scatter(df_data['PTRATIO'], df_tv)
    plt.xlabel('Pupll Teacher Ratio')
    plt.ylabel('Median Value of Home')
    plt.title('Boston Real Eastate')
    
    X_ptratio = np.array(df_data['PTRATIO']).reshape(-1,1)
...
def exercise07():
    ...
    # ------ Place code below here \/ \/ \/ ------
    X = exercise06().reshape(-1,1)
    y = exercise05()[2].reshape(-1,1)
    ...
    y_pred = reg_model.coef_*X_test + reg_model.intercept_
    prediction_space = np.array(y_pred).reshape(-1,1)
    reg_model = X_test
    # ------ Place code above here /\ /\ /\ ------
...
class TestAssignment7(unittest.TestCase):
    def test_exercise07(self):
        rm, ps = exercise07()
        self.assertEqual(len(rm),506)
        self.assertEqual(len(ps),506)
